...
import type { BookingStatus } from "./site";
...
export type NewBooking = Omit<
  Booking,
  "id" | "status" | "depositSimulated" | "adminNote" | "createdAt" | "decidedAt"
>;
...
/** Paiement simulé, en attendant le vrai Stripe. Aucune donnée bancaire n'est
 *  demandée ni stockée : on marque seulement l'intention. */
export async function simulateDeposit(id: string) {
  await updateDoc(doc(db, COLLECTION, id), { depositSimulated: true });
}
